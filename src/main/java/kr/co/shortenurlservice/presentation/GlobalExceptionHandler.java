...
/*
    [아래로 갈수록 더 심각하거나 중요한 로그(더 높은 레벨)]
        * TRACE - 가장 세부적인 수준의 로그로, 코드의 '세부적인' 실행 경로를 추적할 때 사용(양이 많고, 저장공간도 많이 차지함 그래서 기간을 정해서 보관하는게 좋음) - 쿼리 등)
        * DEBUG - 디버깅 목적의 로그로, 개발 중 코드의 상태나 흐름을 이해하기 위해 사용 (주로 찾게될 내용은 DEBUG - 쿼리 등)
        * INFO  - 시스템의 정상적인 운영 상태를 나타내는 정보성 로그로,'중요한 이벤트나 상태 변화'를 기록 (쿼리X:너무 많이 남음, 비즈니스 변화 위주로)
